...
def inference_dense_crop(dataloader, image, gt_mask, index, loss_func, model, device, cfg, record_loss):
    """
    func:
        密集裁剪
    return: 
    ...
    for i in range(0, patch_nums, test_batch_size):
        ...
        if record_loss:
            test_gt_mask_patch = test_gt_mask_patch.unsqueeze(1)  # (B H W) -> (B 1 H W)
            test_patch_batch_mask = test_patch_batch_mask.unsqueeze(1) # (B H W) -> (B 1 H W)
            loss = loss_func(test_gt_mask_patch.cuda(), test_patch_batch_mask)
...
def inference_resize(dataloader, image, gt_mask, index, loss_func, model, device, cfg, record_loss):
    """
    func:
        裁剪
    return: 

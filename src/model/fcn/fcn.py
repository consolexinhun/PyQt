...
__all__ = ['get_fcn32s', 'get_fcn16s', 'get_fcn8s',
           'get_fcn32s_vgg16_voc', 'get_fcn16s_vgg16_voc', 'get_fcn8s_vgg16_voc', "_FCNHead"]
...
def get_fcn32s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='~/.torch/models',
    ...
    num_classes = 1
    model = FCN32s(num_classes, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
    if pretrained:
        from ..base_models.model_store import get_model_file
        device = torch.device(kwargs.get('local_rank', "cuda"))
...
def get_fcn16s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='~/.torch/models',
    ...
    num_classes = 1
    model = FCN16s(num_classes, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
    if pretrained:
        from ..base_models.model_store import get_model_file
...
def get_fcn8s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='~/.torch/models',
    ...
    # from ..data.dataloader import datasets  datasets[dataset].NUM_CLASS
    num_classes = 1
    model = FCN8s(num_classes, backbone=backbone, pretrained_base=pretrained_base, **kwargs)
    if pretrained:
        from ..base_models.model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
...
@MODEL.register("FCN32s_VGG16_VOC")
def get_fcn32s_vgg16_voc(**kwargs):
    return get_fcn32s(dataset='pascal_voc', backbone='vgg16', **kwargs)
